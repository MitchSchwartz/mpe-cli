cmd_logs() {
    ...
    while [ $# -gt 0 ]; do
        case "$1" in
            -n)
                ...
                ;;
            -h | --help)
                echo "Usage: $MPE_CLI_NAME logs <surge|touch|watchdog> [-n N]"
                exit 0
                ;;
            *)
    ...
    case "$target" in
        ...
        watchdog)
            ...
            ;;
        "" | -h | --help)
            echo "Usage: $MPE_CLI_NAME logs <surge|touch|watchdog> [-n N]" >&2
            exit 1
            ;;
        *)
            echo "$MPE_CLI_NAME logs: unknown target: $target (use surge, touch, or watchdog)" >&2
            exit 1
            ;;
    esac
}
